...
socket.on('new_word', function(word_propose, bollsBlack, bollsWhite, currentPlayerNick) {
    ...
    $("#jogadorAtual").html("It's <span class='instaFeed-turn-message'>" + currentPlayerNick + "</span> turn");
...
socket.on('current_player_off', function(nick, nextPlayer) {
    $("#jogadorAtual").html(nick + " está offline, é a vez de <span class='instaFeed-turn-message'>" + nextPlayer + "</span>");
    var myNick = window.sessionStorage.getItem("nick");
...
socket.on('game_is_start', function(statusRoom) {
    ...
    $("#jogadorAtual").html("The first player is: <span class='instaFeed-turn-message'>" + statusRoom.currentPlayer.nick + "</span>");
    $("#player").html(myNick);
...
socket.on('current_player_lost_turn', function(nick, nextPlayer) {
    var myNick = window.sessionStorage.getItem("nick");
    if(myNick==nick)
        $("#jogadorAtual").html("Você perdeu a vez, quem joga agora é <span class='instaFeed-turn-message'>" + nextPlayer + "</span>");
    else if(nextPlayer==myNick)
        $("#jogadorAtual").html(nick + " perdeu a vez, quem joga agora é <span class='instaFeed-turn-message'>" + "você" + "</span>");
    else
        $("#jogadorAtual").html(nick + " perdeu a vez, quem joga agora é <span class='instaFeed-turn-message'>" + nextPlayer + "</span>");
    if (nextPlayer == myNick) {
        unlockSendWord();
    } else {
        lockSendWord();
